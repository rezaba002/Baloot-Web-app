...
//----------------------------------------------------------------------------------------------------------------------
    ...
    public ArrayList<Commodity> getTempCommodities() {
        return tempCommodities;
    }
    public void setTempCommodities(ArrayList<Commodity> tempCommodities) {
        this.tempCommodities = tempCommodities;
    }
    ...
    public ArrayList<ExpiredDiscount> getExpiredDiscounts() {
    ...
    public ArrayList<RateCommodity> getRateCommodities() {
        return rateCommodities;
    }
    public void setRateCommodities(ArrayList<RateCommodity> rateCommodities) {
        this.rateCommodities = rateCommodities;
    }

    private ArrayList<VoteComment> voteComments = new ArrayList<VoteComment>();
    public ArrayList<VoteComment> getVoteComments() {
        return voteComments;
    }
    public void setVoteComments(ArrayList<VoteComment> voteComments) {
    ...
    private ArrayList<BuyListItem> purchasedLists = new ArrayList<BuyListItem>();
    public ArrayList<BuyListItem> getPurchasedLists() {
        return purchasedLists;
    }
    public void setPurchasedLists(ArrayList<BuyListItem> purchasedLists) {
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    public void addUser(HttpClient client, HttpRequest request, Gson gson) throws IOException, InterruptedException {
        ...
        for (User userObj : users) {
            ...
            ArrayList<BuyListItem> buyListItemObj = new ArrayList<BuyListItem>();
            buyListObj.setUsername(userObj.getUsername());
            buyListObj.setBuyListItems(buyListItemObj);
            buyLists.add(buyListObj);
        }
    }
    public void addProvider(HttpClient client, HttpRequest request, Gson gson) throws IOException, InterruptedException {
        HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
        Type providersList = new TypeToken<ArrayList<Provider>>() {
        }.getType();
    ...
    public void addCommodity(HttpClient client, HttpRequest request, Gson gson) throws IOException, InterruptedException {
        HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
        Type commoditiesList = new TypeToken<ArrayList<Commodity>>() {
        }.getType();
    ...
    public void addDiscount(HttpClient client, HttpRequest request, Gson gson) throws IOException, InterruptedException {
        HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
        Type discountsList = new TypeToken<ArrayList<Discount>>() {
        }.getType();
    ...
    public void setFullCurrentUser(String username) {
        for (User userObj : users) {
            if (userObj.getUsername().equalsIgnoreCase(username)){
                currentUser.setUsername(username);
                currentUser.setPassword(userObj.getPassword());
                currentUser.setEmail(userObj.getEmail());
                currentUser.setBirthDate(userObj.getBirthDate());
                currentUser.setAddress(userObj.getAddress());
                currentUser.setCredit(userObj.getCredit());
                for (BuyList buyListObj : buyLists) {
                    if (buyListObj.getUsername().equalsIgnoreCase(username))
                        currentUser.setBuyList(buyListObj);
                }
                ArrayList<BuyListItem> buyListItems = new ArrayList<BuyListItem>();
                currentUser.setPurchasedList(buyListItems);
                for (BuyListItem buyListItemObj : purchasedLists) {
                    if (buyListItemObj.getUsername().equalsIgnoreCase(username))
                        currentUser.getPurchasedList().add(buyListItemObj);
                }
            }
        }
    }
    ...
    public void rateCommodity(String username, int commodityId, float score) {
            ...
            if(buyListObj.getUsername().equalsIgnoreCase(username)) {
                for (Commodity commodityObj : commodities) {
                    if (commodityObj.getId() == commodityId) {
                        ...
                        buyListItem.setUsername(username);
                        buyListItem.setCommodity(commodityObj);
                        buyListItem.setQuantity(1);
                        buyListObj.getBuyListItems().add(buyListItem);
                    }
                }
    ...
    public void updateBuyList(String username, int commodityId, String buyType) {

        for (BuyList buyListObj : buyLists) {
            if (buyListObj.getUsername().equalsIgnoreCase(username)) {
                for (BuyListItem buyListItemObj : buyListObj.getBuyListItems()) {
                    if (buyListItemObj.getCommodity().getId() == commodityId) {
                        if (buyType.equalsIgnoreCase("increase"))
                            buyListItemObj.setQuantity(buyListItemObj.getQuantity() + 1);
                        else if (buyType.equalsIgnoreCase("decrease")) {
                            if (buyListItemObj.getQuantity() == 1) {
    ...
    public void addToPurchasedList(ArrayList<BuyListItem> buyListItems) {
        for (BuyListItem buyListItemObj : buyListItems) {
            for (Commodity commodityObj : commodities) {
                if (buyListItemObj.getCommodity().getId() == commodityObj.getId()) {
//                    buyListItemObj.getCommodity().setInStock(commodityObj.getInStock() - buyListItemObj.getQuantity());
                    commodityObj.setInStock(commodityObj.getInStock() - buyListItemObj.getQuantity());
                }
            }
        }
        for (BuyList buyListObj : buyLists) {
            if (buyListObj.getUsername().equalsIgnoreCase(currentUser.getUsername())) {
                buyListObj.getBuyListItems().clear();
            }
        }
        purchasedLists.addAll(buyListItems);
    }
    public void clearCurrentUser() {
        getCurrentUser().setUsername(null);
        getCurrentUser().setCredit(0);
        getCurrentUser().setBirthDate(null);
        getCurrentUser().setAddress(null);
        getCurrentUser().setEmail(null);
        getCurrentUser().setPassword(null);
        getCurrentUser().setBuyList(null);
        getCurrentUser().setPurchasedList(null);
    }
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    public boolean checkIfCommodityExistsInBuyList(String username, int commodityId) {
        for (BuyList buyListObj : buyLists) {
            if (buyListObj.getUsername().equalsIgnoreCase(username)) {
                for (BuyListItem buyListItemObj : buyListObj.getBuyListItems()) {
                    if (buyListItemObj.getCommodity() == null)
                        continue;
                    if (buyListItemObj.getCommodity().getId() == commodityId) {
                        return true;
                    }
    ...
    public boolean checkIfLoginCredentialsIsValid(String username, String password) {
        for (User userObj : users) {
