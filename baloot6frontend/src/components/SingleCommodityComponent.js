...
const SINGLE_COMMODITY_API = "http://localhost:8080/commodities";
...
class SingleCommodityComponent extends React.Component {
    ...
    componentDidMount() {
        ...
        localStorage.setItem('commodityProviderName', "");
        let url = window.location.pathname;
        let urlTokens = url.split('/');
        let commodityId = urlTokens[2];

        UserService.getUserBuyList().then((response) => {
            this.setState({numberOfBuyListItems: response.data.length})
        })

        axios.get(SINGLE_COMMODITY_API + "/" + commodityId)
            .then(response => {
                this.setState({commodity: response.data})
            })
        setTimeout(() => {
            axios.get(SINGLE_PROVIDER_API + "/" + this.state.commodity.providerId)
                .then(response => {
                    // localStorage.setItem('commodityProviderName', response.data.name);
                    this.setState({providerName : response.data.name})
                         ...
                         className="bg-white container-fluid px-4 px-xl-5 py-0 mb-4 mb-xl-5 d-flex flex-column flex-nowrap align-items-center">
                                    ...
                                    <div id="commodity" className="main-item col-11 row g-0">
                                        <div id="commodity-image" className="col-6 px-2">
                                            <div>
                                                <img src={this.state.commodity.image} alt="commodity" className="col-12"/>
                                            </div>
                                        </div>
                                        <div id="commodity-details" className="col-6 px-2">
                                            <div id="commodity-name"
                                                 className="color-brown fw-bold">{this.state.commodity.name}</div>
                                            <div id="commodity-inStock"
                                                 className="color-red fw-semibold">{this.state.commodity.inStock} left in
                                                stock
                                            </div>
                                            <div id="commodity-provider" className="fw-semibold">
                                                by
                                                <a style={{cursor: 'pointer'}} onClick={() => this.loadProvider(this.state.commodity.providerId)}
                                                   className="color-blue"> {this.state.providerName} </a>
                                            </div>
                                            <div id="commodity-categories" className="color-brown">
                                                Category(s)
                                                <ul>
                                                    <li>{this.state.commodity.categories}</li>
                                                </ul>
                                            </div>
                                            <div id="price-add"
                                                ...
                                                <span id="commodity-price"
                                                      className="color-brown fw-semibold me-auto">{this.state.commodity.price}T</span>
                                                <AddToCartCardInSingleCommodity commodityId={this.state.commodity.id} inStock={this.state.commodity.inStock}/>
                                            </div>
                                            ...
                                            <div id="commodity-rating">
                                                ...
                                                <span className="color-brown fs-3 fw-semibold">{this.state.commodity.rating}</span>
                                                <span className="color-grey fw-semibold">({this.state.commodity.numberOfVoters})</span>
                                            </div>
                                        </div>
                                    </div>
                        {/*{*/}
                                        ...
                                        <div className="card h-100 px-2 py-3">
                                            <div className="card-body py-0">
                                                <div className="card-title color-brown m-0 fw-semibold">
                                                    <div style={{cursor: 'pointer'}} onClick={() => this.loadCommodity(suggested.id)}>
                                                        {suggested.name}
                                                    </div>
                                                </div>
